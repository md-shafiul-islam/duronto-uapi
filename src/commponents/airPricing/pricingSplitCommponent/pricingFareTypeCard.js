...
const PricingFareTypeCard = (params) => {
  ...
  const [cstDetails, setCstDetails] = useState({name:"", cancelPenalty:{}, changePenalty:{}});
  ...
  useEffect(() => {
    ...
    if (params.airSolution) {
      ...
      let cstLDetails = {name:"", cancelPenalty:{}, changePenalty:{}, baggageAllowances:{}};
      params.airSolution.airPricingInfo &&
        params.airSolution.airPricingInfo.map((priceItem, prcIdx) => {
          ...
          if (priceItem.passengerType[0]) {
            ...
            priceItem.bookingInfo&&priceItem.bookingInfo.map((book, idx)=>{
